...
export function useReActAgent(
    options: UseReActAgentOptions = {}
): ReActAgentHookReturn {
    const { toast } = options;
    // ============================================
    // State
    // ============================================
    ...
    // Track if session was just loaded (to avoid inheriting previous session's state)
    // When true, next task should use continueSession: false to start fresh
    ...
    const updateMessage = useCallback(
        (id: string, updates: Partial<AgentChatMessage>) => {
            setMessages((prev) =>
                prev.map((msg) =>
    ...
    const updateExecutionStep = useCallback(
        (stepId: string, updates: Partial<ExecutionStep>) => {
            const msgId = currentTaskMsgRef.current;
            if (!msgId) return;
    ...
    const refreshSessions = useCallback(async () => {
        ...
        try {
            ...
            const currentId =
                await window.electronAPI.agent.getCurrentSession();
            ...
            try {
                ...
                if (targetThreadId) {
                    const checkpointList =
                        await window.electronAPI.agent.getCheckpointHistory(
                            targetThreadId
                        );
                    setCheckpoints(checkpointList);
                } else {
                    const checkpointList =
                        await window.electronAPI.agent.listCheckpoints();
                    setCheckpoints(checkpointList);
                }
            } catch (e) {
    ...
    const connectBrowser = useCallback(
        async (url?: string) => {
            if (!hasElectronAPI) {
                addMessage(
                    'agent',
                    '⚠️ Electron API not available. Running in browser mode.',
                    'error'
                );
                return;
            }
    ...
    const executeTask = useCallback(
        async (task: string) => {
            ...
            try {
                // Determine if we should continue session or start fresh
                // After loading a session, the first task should NOT continue
                // to avoid inheriting executionVariables from previous runs
                const shouldContinue =
                    !!currentSessionId && !sessionJustLoadedRef.current;
                ...
                // Execute task with current session ID
                const result = await window.electronAPI.agent.executeTask(
                    task,
                    {
    ...
    const stopTask = useCallback(async () => {
        ...
        try {
            ...
            addMessage(
                'system',
                `✗ 停止失败: ${e instanceof Error ? e.message : 'Unknown'}`,
                'error'
            );
        }
    ...
    const sendMessage = useCallback(
        async (content: string, editCheckpointId?: string) => {
            // Sanitize input
            ...
            try {
                ...
                if (editCheckpointId) {
                    ...
                    const conversation =
                        await window.electronAPI.agent.getConversation();
    ...
    const createSession = useCallback(
        async (name: string, description?: string) => {
            ...
            try {
                const result = await window.electronAPI.agent.createSession(
                    name,
                    description
                );
                ...
                toast?.error(
                    `错误: ${e instanceof Error ? e.message : 'Unknown'}`
                );
            }
        },
    ...
    const loadSession = useCallback(
        async (sessionId: string) => {
            ...
            try {
                const result = await window.electronAPI.agent.loadSession(
                    sessionId
                );
                if (result.success) {
                    ...
                    // Mark that session was just loaded - next task should NOT continue
                    // This ensures executionVariables and other caches are reset
                    ...
                    if (conversation && conversation.length > 0) {
                        ...
                        const sessionName =
                            session?.name || sessionId.substring(0, 12);
                        ...
                        if (expectedMessageCount > 0) {
                            toast?.error(
                                `已切换到会话: ${sessionName} (历史消息已丢失，可能因为使用了内存模式)`
                            );
                        } else {
                            toast?.info(
                                `已切换到会话: ${sessionName} (无历史消息)`
                            );
                        }
                    }
    ...
    const deleteSession = useCallback(
        async (sessionId: string) => {
            ...
            try {
                const success = await window.electronAPI.agent.deleteSession(
                    sessionId
                );
                if (success) {
                    toast?.success('会话已删除');
    ...
    const createCheckpoint = useCallback(
        async (name: string, description?: string) => {
            ...
            try {
                const result = await window.electronAPI.agent.createCheckpoint(
                    name,
                    description
                );
                if (result.success) {
    ...
    const restoreCheckpoint = useCallback(
        async (checkpointId: string, threadId?: string) => {
            if (!hasAgentAPI) return;
            ...
            try {
                const result = await window.electronAPI.agent.restoreCheckpoint(
                    targetThreadId,
                    checkpointId
                );
    ...
    const deleteCheckpoint = useCallback(
        async (checkpointId: string) => {
            ...
            try {
                const success = await window.electronAPI.agent.deleteCheckpoint(
                    checkpointId
                );
                if (success) {
    ...
    useEffect(() => {
        ...
        const unsubscribe =
            window.electronAPI.onOperationRecorded(handleOperation);
        ...
        if (window.electronAPI.onBrowserStatusChanged) {
            unsubscribeStatus = window.electronAPI.onBrowserStatusChanged(
                (status) => {
                    setIsConnected(status.connected);
                }
            );
    ...
    useEffect(() => {
        ...
        unsubscribers.push(
            window.electronAPI.agent.onStepStarted((data: unknown) => {
                ...
                if (stepData?.step) {
                    ...
                    const stepType =
                        stepData.node === 'planner'
                            ? 'planner'
                            : stepData.node === 'observe'
                            ? 'observe'
                            : 'codeact';
        ...
        if (window.electronAPI.agent.onThinkingUpdate) {
            unsubscribers.push(
                window.electronAPI.agent.onThinkingUpdate(
                    (data: {
                        stepId: string;
                        thought: string;
                        instruction: string;
        ...
        if (window.electronAPI.agent.onCodeUpdate) {
            unsubscribers.push(
                window.electronAPI.agent.onCodeUpdate(
                    (data: {
                        stepId: string;
                        code: string;
                        instruction: string;
