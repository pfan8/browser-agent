    ...
    async (_event, config: { apiKey: string; baseUrl?: string }) => {
        try {
            const keyPreview = config.apiKey
                ? `${config.apiKey.substring(0, 10)}...${config.apiKey.slice(
                      -4
                  )}`
                : 'none';
            log.debug(
                `set-llm-config called: keyPreview=${keyPreview}, baseUrl=${
            ...
            for await (const event of agentInstance.streamTask(
                ...
                if (event.node === 'planner') {
                    ...
                    if (pendingPlannerStepId) {
                        const duration =
                            Date.now() -
                            (stepStartTimes.get(pendingPlannerStepId) ||
                                Date.now());
                        safeSend('agent-step-completed', {
                            step: {
                    ...
                    if (
                        ...
                        state.currentInstruction !== lastInstruction
                    ) {
                ...
                if (event.node === 'codeact') {
                    ...
                    const currentActionCount =
                        event.state.actionHistory?.length || 0;

                    // New action was added
                    if (currentActionCount > lastActionCount) {
                        const newAction =
                            event.state.actionHistory![currentActionCount - 1];
                        ...
                        if (newAction) {
                            ...
                            safeSend('agent-step-started', {
                                step: {
                                    id: stepId,
                                    description: `⚡ ${instruction.substring(
                                        0,
                                        50
                    ...
                    if (
                        ...
                        const lastAction =
                            event.state.actionHistory[
                                event.state.actionHistory.length - 1
                            ];
                            ...
                            if (lastAction.result.success) {
                                safeSend('agent-step-completed', {
                                    step: {
                                        id: pendingCodeActStepId,
                                        description: `✅ ${
                                            lastAction.reasoning?.substring(
                                                0,
                                                40
                                            ) || '执行成功'
                                        }`,
...
ipcMain.handle('agent-load-session', async (_event, sessionId: string) => {
    try {
        ...
        // This ensures subsequent tasks use the correct session context
        agentInstance.setCurrentThreadId(sessionId);

        // Reset session state to clear temporary caches (executionVariables, etc.)
        // This ensures new tasks in this session start with clean state
        agentInstance.resetSessionState();
...
ipcMain.handle('agent-list-checkpoints', async (_event, threadId?: string) => {
    try {
        ...
        const history = await agentInstance.getCheckpointHistory(
            currentThreadId
        );
 ...
 *
 * Uses regex to find JSON in the content, matching the parsing logic
 * in planner-node.ts parsePlannerResponse. LLM responses may contain
 * text before/after the JSON object.
 */
function extractCompletionFromAIMessage(content: string): string | null {
    // Use regex to extract JSON from anywhere in the content
    // This matches the logic in parsePlannerResponse
 ...
 */
function formatLangGraphMessage(
    msg: any,
    idx: number
...
ipcMain.handle(
    'agent-delete-checkpoint',
    async (_event, _checkpointId: string) => {
