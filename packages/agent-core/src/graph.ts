...
export class BrowserAgent {
    ...
    private compiledGraph: ReturnType<
        ReturnType<typeof createAgentGraph>['compile']
    > | null = null;
    ...
    async getCheckpointHistory(
        ...
        try {
            ...
            for await (const snapshot of this.compiledGraph.getStateHistory(
                ...
                const checkpoint = this.snapshotToHistoryItem(
                    threadId,
                    snapshot
                );
                if (checkpoint) {
    ...
    async getStateAtCheckpoint(
        ...
        try {
            ...
            if (state.messages && Array.isArray(state.messages)) {
                state.messages = (state.messages as unknown[]).map(
                    coerceToBaseMessage
                );
            }
    ...
    /**
     * Set current thread ID (for session switching)
    ...
    /**
     * Reset session state for starting fresh in a loaded session
     * Clears temporary caches like executionVariables while keeping the session context
     * This should be called after loading a session to ensure clean state for new tasks
     */
    resetSessionState(): void {
        log.info('Resetting session state', { threadId: this.currentThreadId });
        // Note: The actual state reset happens in streamTask/executeTask
        // when continueSession is false - executionVariables, messages, etc.
        // are initialized to empty values. This method is primarily for
        // any additional in-memory caches that need clearing.
    }
