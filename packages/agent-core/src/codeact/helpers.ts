...
export async function executeCode(
    ...
    try {
        ...
        const result: CodeExecutionResult = await Promise.race([
            execPromise,
            timeoutPromise,
        ]);
...
export function summarizeResult(result: unknown): string {
    ...
    if (typeof result === 'object') {
        ...
        const summary = keys
            .map((k) => `${k}: ${String(obj[k]).slice(0, 50)}`)
            .join(', ');
        return summary || '操作完成';
    }
 ...
 * Note: state.observation may be null on first execution, so we provide a fallback
 */
export function buildNewObservation(state: AgentState, execResult: CodeResult) {
    // Provide fallback empty object when observation is null to prevent runtime error
    // when spreading null (TypeError: Cannot spread non-iterable instance)
    const baseObservation = state.observation || {};
